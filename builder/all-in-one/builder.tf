...
#  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "for x in *.json ; do docker run --rm -e PACKER_LOG=1 -e PACKER_LOG_PATH=$x.log -d --name $x --privileged --cap-add=ALL -v /lib/modules:/lib/modules -v $PWD:/opt/ -e AWS_ACCESS_KEY=${var.AWS_ACCESS_KEY} -e AWS_SECRET_KEY=${var.AWS_SECRET_KEY} -w /opt/ goffinet/packer-qemu build $x ; done",
