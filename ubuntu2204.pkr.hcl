...
source "qemu" "ubuntu2204" {
  accelerator      = "kvm"
  boot_command     = ["c<wait>linux /casper/vmlinuz --- autoinstall ds=\"nocloud;s=http://{{ .HTTPIP }}:{{ .HTTPPort }}/http/${var.name}${var.version}/\"<enter><wait>", "initrd /casper/initrd<enter><wait>", "boot<enter><wait>"]
  disk_cache       = "none"
