#Generated by packer-kvm/build-packer-templates.yaml at 2024-10-19T13:20:53Z
...
source "qemu" "ubuntu2204" {
  accelerator      = "kvm"
  boot_command     = ["c<wait>linux /casper/vmlinuz --- autoinstall ds=\"nocloud-net;seedfrom=http://{{ .HTTPIP }}:{{ .HTTPPort }}/http/${var.name}${var.version}/${var.config_file}"<enter><wait>", "initrd /casper/initrd<enter><wait>", "boot<enter>", "<enter><f10><wait>"]
  disk_cache       = "none"
