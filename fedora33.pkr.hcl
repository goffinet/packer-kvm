...
variable "config_file" {
  type    = string
  default = "fedora33-kickstart.cfg"
}
...
variable "iso_checksum" {
  type    = string
  default = "1f1f018e78f0cc23d08db0c85952344ea5c200e67b672da5b07507c066a52ccf"
}
...
variable "iso_urls" {
  type    = string
  default = "http://ftp.belnet.be/mirror/fedora/Fedora/linux/releases/33/Server/x86_64/iso/Fedora-Server-netinst-x86_64-33-1.2.iso"
}
...
source "qemu" "{{user_`name`}}{{user_`version`}}" {
  accelerator          = "kvm"
  boot_command         = ["<tab> linux text net.ifnames=0 biosdevname=0 ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/http/{{user `config_file`}}<enter><wait>"]
  boot_wait            = "40s"
  disk_cache           = "none"
